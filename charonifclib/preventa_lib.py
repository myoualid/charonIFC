...
import logging
try:
    from pysqlite2 import dbapi2 as sqlite3 #python 2.7
except ImportError:
    import sqlite3 #python 3.2
...
class Preventivo:
    ...
    def __init__(self, file_preventivo, lib_settings=None, dati_generali = None):
        ...
        logger.info(_("Linguaggio e codifica locale: {0}").format(locale.getlocale()))
...
CREATE TABLE IF NOT EXISTS Computo (id INTEGER PRIMARY KEY, 
...
CREATE TABLE IF NOT EXISTS Epu (Tariffa PRIMARY KEY, 
                                Supercapitolo INTEGER, 
                                Capitolo INTEGER, 
                                Subcapitolo INTEGER, 
...
CREATE TABLE IF NOT EXISTS Analisi (id INTEGER PRIMARY KEY, 
                                    Tariffa TEXT, 
                                    Codice TEXT, 
                                    Descrizione_codice TEXT, 
                                    Unita_misura TEXT, 
...
CREATE TABLE IF NOT EXISTS Supercapitolo (id INTEGER PRIMARY KEY,
                                          Nome TEXT,
                                          Descrizione TEXT,
                                          Aumento_prezzi REAL);
CREATE TABLE IF NOT EXISTS Capitolo      (id INTEGER PRIMARY KEY,
                                          Nome TEXT,
                                          Descrizione TEXT,
                                          Aumento_prezzi REAL);
CREATE TABLE IF NOT EXISTS Subcapitolo   (id INTEGER PRIMARY KEY,
                                          Nome TEXT,
                                          Descrizione TEXT,
                                          Aumento_prezzi REAL);

/*Creazione delle tavole Categorie Computo */
CREATE TABLE IF NOT EXISTS Supercategoria (id INTEGER PRIMARY KEY,
                                           Nome TEXT,
                                           Descrizione TEXT,
                                           Aumento_prezzi REAL);
CREATE TABLE IF NOT EXISTS Categoria      (id INTEGER PRIMARY KEY,
                                           ...
                                           Descrizione TEXT,
                                           Aumento_prezzi REAL);
CREATE TABLE IF NOT EXISTS Subcategoria   (id INTEGER PRIMARY KEY,
                                           Nome TEXT,
                                           Descrizione TEXT,
                                           Aumento_prezzi REAL);

/*Creazione delle tavole per i Dati Generali */
CREATE TABLE IF NOT EXISTS DatiGenerali (Ricarico REAL,
                                         Manodopera REAL, 
                                         Sicurezza REAL, 
                                         Valuta TEXT, 
                                         Nome_lavoro TEXT, 
...
CREATE TABLE IF NOT EXISTS Manodopera (id INTEGER PRIMARY KEY, 
                                       DescOper TEXT,
                                       CostOper REAL,
                                       PercOper REAL,
                                       Note TEXT);
...
CREATE TRIGGER IF NOT EXISTS Update_PU_Epu AFTER UPDATE OF Ricarico, Tempo_inst, Costo_materiali, Sicurezza ON Epu FOR EACH ROW 
      BEGIN 
        UPDATE Epu SET Prezzo_unitario = (((new.Costo_materiali * new.Ricarico)+ 
                                         (new.Tempo_inst/60.0 * 
                                         (SELECT Manodopera FROM DatiGenerali)))*
                                         (1+(new.Sicurezza/100)) )
...
CREATE TRIGGER IF NOT EXISTS Update_Pr_tot_after_mod_Analisi AFTER UPDATE OF Quantita, Prezzo_unitario, Sconto, Accessori ON Analisi FOR EACH ROW 
      BEGIN 
        UPDATE Analisi SET Prezzo_totale = new.Quantita * new.Prezzo_unitario * (1- (new.Sconto/100)) + new.Accessori 
        WHERE id = new.id;
      END;
...
CREATE TRIGGER IF NOT EXISTS Cancella_Supercategoria AFTER DELETE ON Supercategoria FOR EACH ROW  
      BEGIN 
        UPDATE Computo SET Supercategoria = 0 
        WHERE Supercategoria NOT IN (SELECT id FROM Supercategoria); 
      END;
CREATE TRIGGER IF NOT EXISTS Cancella_Categoria AFTER DELETE ON Categoria FOR EACH ROW  
      BEGIN 
        UPDATE Computo SET Categoria = 0 
...
CREATE TRIGGER IF NOT EXISTS Cancella_Subcategoria AFTER DELETE ON Subcategoria FOR EACH ROW  
      BEGIN 
        UPDATE Computo SET Subcategoria = 0 
        WHERE Subcategoria NOT IN (SELECT id FROM Subcategoria); 
      END;
CREATE TRIGGER IF NOT EXISTS Cancella_Supercapitolo AFTER DELETE ON Supercapitolo FOR EACH ROW  
      BEGIN 
        UPDATE Epu SET Supercapitolo = 0 
        WHERE Supercapitolo NOT IN (SELECT id FROM Supercapitolo); 
      END;
CREATE TRIGGER IF NOT EXISTS Cancella_Capitolo AFTER DELETE ON Capitolo FOR EACH ROW  
      BEGIN 
        UPDATE Epu SET Capitolo = 0 
        WHERE Capitolo NOT IN (SELECT id FROM Capitolo); 
      END;
CREATE TRIGGER IF NOT EXISTS Cancella_Subcapitolo AFTER DELETE ON Subcapitolo FOR EACH ROW  
      ...
      END;""")
        logger.debug(_("TABLE e TRIGGER creati nel database: %s") % self.__FileDB)
    ...
    def __delete_triggers(self):
        ...
        logger.debug(_("DROP dei TRIGGER esistenti nel database %s effettuato") % self.__FileDB)
    ...
    def __inizialize_database(self):
        ...
        if len(self.c.fetchall()) == 0:
            self.c.execute("""INSERT INTO DatiGenerali VALUES (
                                          :ricarico, 
                                          :manodopera,
                                          :sicurezza,
                                          :valuta, 
        ...
        try:
            ...
            logger.debug("TABLE 'Computo' aggiornata, aggiunte colonne: 'Tipo_lavori', 'Cat_appalto', 'Image_art'")
            ...
            try:
                ...
                logger.debug(_("Colonne 'Tipo_lavori', 'Cat_appalto', 'Image_art' nella TABLE 'Computo' esistenti, aggiornamento non necessario."))
            except sqlite3.OperationalError:
    ...
    def __retrive_dati_generali(self):
        ...
        dati = {"ricarico":lista[0][0], 
                "manodopera":lista[0][1], 
                "sicurezza":lista[0][2], 
                "valuta":lista[0][3], 
                "nome":lista[0][4], 
                "indirizzo":lista[0][5], 
    ...
    def delete_database(self):
        ...
        try:
            ...
            logger.debug(_("Tutte le tavole esistenti nel database %s sono state eliminate") % self.__FileDB)
...
# il rollback e chiudere la connessione con il database
    ...
    def save_database (self):
        ...
        try:
            ...
            logger.debug(_("Modifiche al database %s salvate") % self.__FileDB)
    ...
    def rollback_database (self):
        ...
        try:
            ...
            logger.debug(_("Modifiche al database '%s' annullate") % self.__FileDB)
    ...
    def connection_shutdown (self):
        ...
        self.c.close()
        logger.debug(_("Connessione al database %s chiusa") % self.__FileDB)
        return True
...
# Nota: *****
    ...
    def update_dati_generali (self, nome=None, cliente=None, redattore=None, 
        ...
        try:
            ...
            dati = self.__retrive_dati_generali()
            self.set_dati_generali(dati)
            logger.debug(_("Dati generali aggiornati in 'DatiGenerali'"))
            return result
    ...
    def update_ricarico_epu (self, new_ricarico):
        ...
        try:
            ...
            logger.debug(_("Ricarico aggiornato in 'Epu' e 'DatiGenerali'"))
            return result
    ...
    def update_sicurezza_epu (self, new_sicurezza):
        ...
        try:
            ...
            logger.debug(_("Sicurezza aggiornata in 'Epu' e 'DatiGenerali'"))
            return result
...
################################################################################
    ...
    def insert_manodopera (self, nome=None, costo=None, perc=None, note=None):
        ...
        try:
            ...
            logger.debug(_("Rigo manodopera '%s' inserito.") % nome)
            return result
    ...
    def update_manodopera (self, key, nome= None, costo=None, perc=None, note=None):
        ...
        try:
            ...
            logger.debug(_("Rigo manodopera '%s' aggiornato.") % nome)
            return result
    ...
    def delete_manodopera (self, key):
        ...
        try:
            ...
            logger.debug(_("Rigo manodopera '%s' cancellato.") % key)
            return result
    ...
    def calcola_media_manodopera (self, update_dg = True):
        ...
        if len(lista) == 0:
            logger.debug(_("Tabella manodopera vuota, manodopera non aggiornata"))
            return None
        ...
        if perc_tot != 0:
            ...
            logger.debug(_("Percentuale totale delle righe manodopera nulla, manodopera non aggiornata"))
            return None
        # aggiorna i dati generali
        if update_dg:
            self.update_dati_generali (manodopera= mo)
            logger.debug(_("Valore della manodopera aggiornata: '%s'") % mo)
        return mo
...
# ma possono anche essere create o eliminate separatamente con i seguenti metodi
    ...
    def insert_capitoli_categorie (self, Tipo, Nome, Descrizione = None, Aumento_prezzi = None, codice = None):
        ...
        if Descrizione is None:
            ...
            raise PreventiviError(_("Errore nella tabella {0} 'Descrizione' = {1} deve essere una stringa").format(str(Tipo), str(Descrizione)))
            Descrizione = str()
        if Aumento_prezzi is None:
            Aumento_prezzi = 0.0
        elif type(Aumento_prezzi) != int:
            raise PreventiviError(_("Errore nella tabella {0} 'Aumento_prezzi' = {1} deve essere un numero intero").format(str(Tipo), str(Aumento_prezzi)))
            Aumento_prezzi = 0.0
        ...
        try:
            ...
            logger.debug(_("Inserita nella tabella '{0}': {1}").format(Tipo, Nome))
        except:
             raise PreventiviError(_("Errore di esecuzione inserimento nella tabella {0} Capitoli e Categorie: {1}").format(str(Tipo), str(Nome)))
             return False
        return True
    ...
    def update_capitoli_categorie (self, key, Tipo, Nome, Descrizione=None, Aumento_prezzi=None):
        ...
        try:
            ...
            logger.debug("Tabella aggiornata '{0}'colonna 'id': {1}".format(Tipo, key))
        except:
             raise PreventiviError(_("Errore di aggiornamento da tabella {0} nome {1} id {2}").format(str(Tipo), str(Nome), str(key)))
             result = False
        return result
    ...
    def delete_capitoli_categorie (self, Tipo, key, column = None):
        ...
        try:
            ...
            logger.debug(_("Tabella cancellata '{0}'colonna 'id': {1}").format(Tipo, key))
        except:
            raise PreventiviError(_("Errore eliminazione da tabella {0} colonna {1} voce {2}").format(str(Tipo), str(column), str(key)))
            result = False
        return result
...
# Nota: le operazioni avvengono sempre su oggetti appartenenti alla classe Articoli
    ...
    def insert_articoli_epu (self, verifica_tariffa = False, *lista_articoli):
        ...
        for articolo in lista_articoli:
            ...
            try:
                ...
                logger.debug(_("Inserito articolo tabella '{0}' col 'Tariffa': {1}").format('Epu', articolo.tariffa))
    ...
    def delete_articoli_epu (self, *lista_articoli):
        ...
        for articolo in lista_articoli:
            try:
                ...
                logger.debug(_("Cancellato articolo tabella '{0}' col 'id': {1}").format('Epu', articolo.tariffa))
        return result
    ...
    def update_articolo_epu (self, tariffa_esistente, articolo):
        ...
        try:
            self.c.execute("""UPDATE Epu SET Tariffa=:Tar, Supercapitolo=:SupCap, 
                                  Capitolo=:Cap, 
                                  Subcapitolo=:SubCap,
                                  Descrizione_codice=:DesCod, 
                                  Descrizione_voce=:DesVoc, 
                                  Descrizione_estesa=:DesEst, 
                                  Unita_misura=:UM, 
                                  Ricarico=:Ric, 
                                  Tempo_inst=:Temp, 
                                  Costo_materiali=:CostoMat,
                                  Sicurezza=:Sicurezza, 
                                  Cod_analisi=:cod_listino, 
                                  Note=:Note, 
                                  CostoMat_1=:CostoMat1, 
                                  CostoMat_2=:CostoMat2, 
                                  CostoMat_3=:CostoMat3, 
                                  CostoMat_4=:CostoMat4
                              WHERE Tariffa =:old_Tariffa;""", rigo)
        except:
            raise PreventiviError(_("Errore dell'aggiornamento dell'articolo: %s in Epu") % str(articolo))
            result = False
        finally:
            logger.debug(_("Aggiornato articolo tabella '{0}' col 'id': {1}").format('Epu', articolo.tariffa))
        return result
    ...
    def copia_articolo_epu (self, tariffa, supercategoria = None, categoria = None, subcategoria = None, copia_nomi_capitoli= False):
        ...
        if len(lista_epu) == 0:
             logger.debug(_("Impossibile copiare la tariffa '%s', la tariffa non esiste nel database") % tariffa)
             return None
        e = lista_epu[0]
        articolo = ArticoloComputo(database=self.db, 
                       supercapitolo = e[1], 
                       capitolo = e[2], 
                       subcapitolo = e[3],
                       ...
                       tariffa= tariffa, 
                       codice=None, 
                       descrizione_codice=e[4], 
                       descrizione_voce=e[5], 
                       descrizione_estesa=e[6],
                       unita_misura=e[7], 
                       quantita=0, 
                       ricarico=e[8], 
                       tempo_inst=e[9], 
                       costo_materiali=e[10], 
                       prezzo_unitario=e[11], 
                       sicurezza=e[12],
                       cod_listino=e[13], 
                       note=e[14], 
                       data= None, 
    ...
    def update_articoli_epu (self, supercapitolo, capitolo, subcapitolo, un_mis, ric, temp,
        ...
        for articolo in lista_articoli:
            ...
            try:
                self.c.execute("""UPDATE Epu SET Supercapitolo=:SupCap, 
                                                 Capitolo=:Cap,
                                                 Subcapitolo=:SubCap, 
                                                 Unita_misura=:UM, 
                                                 Ricarico=:Ric, 
    ...
    def scambia_prezzi_articoli_epu (self, costo_partenza, costo_arrivo, *lista_articoli):
        ...
        for articolo in lista_articoli:
            ...
            try:
                self.c.execute("""UPDATE Epu SET Costo_materiali=:CostoMat, 
                                                 CostoMat_1=:CostoMat1, 
                                                 CostoMat_2=:CostoMat2, 
                                                 CostoMat_3=:CostoMat3, 
                                                 CostoMat_4=:CostoMat4 
                ...
                logger.debug(_("Aggiornato prezzo articolo tabella '{0}' col 'id': {1}").format('Epu', articolo.tariffa))
        return False
...
# Nota: le operazioni avvengono sempre su oggetti appartenenti alla classe Articoli
    ...
    def insert_articoli_computo (self, *lista_articoli):
        ...
        for articolo in lista_articoli:
            ...
            try:
                ...
                logger.debug("Inserito articolo tabella '{0}', 'id' {1}, 'Tariffa' {2}".format(
    ...
    def update_articolo_computo (self, prymary_key, articolo, new_id=None):
        ...
        try:
            ...
            logger.debug(_("Aggiornato articolo tabella '{0}', 'id' {1}, 'Tariffa' {2}").format(
                          'Computo', articolo.codice, articolo.tariffa))
        return result
    ...
    def delete_articoli_computo (self, *lista_articoli):
        ...
        for articolo in lista_articoli:
            try:
                ...
                logger.debug(_("Cancellato articolo tabella '{0}', 'id' {1}, 'Tariffa' {2}").format(
                              'Computo', articolo.codice, articolo.tariffa))
        return result
    ...
    def nuovo_articolo_computo (self, tariffa, descrizione_codice, descrizione_voce=None, descrizione_estesa=None, 
        ...
        if categoria is None: categoria = 0
        if subcategoria is None: subcategoria = 0
        articolo = ArticoloComputo(self.db, 
                                   supercapitolo = 0, 
                                   capitolo = 0, 
                                   subcapitolo = 0, 
                                   supercategoria = supercategoria, 
    ...
    def update_lista_articoli (self, colonna, nuovo_campo, *lista_articoli):
        ...
        for articolo_esistente in lista_articoli:
            ...
            if colonna in colonne_computo:
                self.c.execute("""UPDATE Computo SET :column =:new_value 
                                         WHERE id =:id;""", dict_modifiche)
            elif colonna in colonne_epu:
                self.c.execute("""UPDATE Epu SET :column =:new_value 
                                         WHERE id =:tariffa;""", dict_modifiche)
            else: 
                return False
            return True
    ...
    def delete_row_categoria (self, tipo, id_categoria):
        ...
        try:
            ...
            logger.debug("Tutti gli articoli della '%s' numero '%d'cancellati dal database" % (tipo, id_categoria))
            return True
        except: 
            raise PreventiviError(_("Errore: impossibile cancellare gli articoli della '{0}' numero '{1}'").format(tipo, id_categoria))
            return False
...
################################################################################
    ...
    def insert_articoli_listino (self, tariffa, *lista_listino):
        ...
        for articolo_listino in lista_listino:
            ...
            #print self.__verifica_codice_listino_esistente(articolo_listino)
            if not self.__verifica_codice_listino_esistente(articolo_listino):
                ...
                try:
                    ...
                    logger.debug(_("Inserito articolo tabella '{0}' con 'Tariffa': '{1}' con 'Codice': '{2}'").format(
                                    'Analisi', articolo_listino.tariffa, articolo_listino.codice))
            # se l'elemento di listino è già esistente non lo inserisce
            else:
                logger.warning(_("Articolo tab 'Analisi' esistente per la Tariffa {0}: {1} \n l'articolo duplicato non verrà inserito").format(articolo_listino.tariffa, str(articolo_listino)))
                result = False
        return result
    ...
    def delete_articoli_listino (self, *lista_listino):
        ...
        for art_listino in lista_listino:
            try:
                ...
                logger.debug(_("Cancellato articolo tabella '{0}' col 'id': {1}").format('Analisi', art_listino.nr))
        return result
    ...
    def update_articolo_listino (self, prymary_key, articolo):
        ...
        rigo["id"] = prymary_key
        try:
            self.c.execute("""UPDATE Analisi SET Codice=:Codice, 
                                                 Descrizione_codice=:DesCod, 
                                                 Unita_misura=:UM, 
                                                 Quantita=:Quantita, 
            ...
            logger.debug(_("Aggiornato articolo tabella '{0}' col 'id': {1}").format('Analisi', prymary_key))
        return result
    ...
    def update_lista_art_listino (self, tariffa, quantita, prezzo_unitario, 
        ...
        for articolo in lista_articoli:
            ...
            try:
                ...
                logger.debug(_("Aggiornato articolo tabella '{0}' col 'id': {1}").format('Analisi', articolo.nr))
        return result
    ...
    def nuovo_articolo_listino (self, tariffa, codice, descrizione_codice, 
        ...
        articolo = ArticoloListino(tariffa = tariffa, 
                                   codice = codice, 
                                   descrizione_codice = descrizione_codice, 
...
################################################################################
    def ricalcolo_generale(self):
        ...
        try:
            ...
            logger.debug(_("Ricalcolo generale di tutti i prezzi effettuato"))
    ...
    def ricalcolo_prezzo_tariffa(self, tariffa):
        ...
        try:
            ...
            logger.debug(_("Ricalcolo prezzo tariffa {0} effettuato").format(tariffa))
            return True
        ...
        for tariffa in lista_tariffe:
            ...
            if len(self.get_articoli_listino(tariffa)) > 0 and varia_costi and \
                ...
                try:
                    ...
                    logger.debug(_("Modifica '{0}' tabella '{1}' con tariffa '{2}'").format('Prezzo_unitario', 'Analisi', tariffa))
                    logger.debug(_("Modifica '{0}' tabella '{1}' con tariffa '{2}'").format('Accessori', 'Analisi', tariffa))
            # aumenta/diminuisci il costo unitario di ogni voce in lista
                ...
                try:
                    ...
                    logger.debug(_("Modifica '{0}' tabella '{1}' con tariffa '{2}'").format('Costo_materiali', 'Epu', tariffa))
            ...
            if varia_tempi:
                try:
                    ...
                    logger.debug(_("Modifica '{0}' tabella '{1}' con tariffa '{2}'").format('Tempo_inst', 'Epu', tariffa))
            # informazioni di debug
            logger.debug(_("Fine modifica tabella {0} articolo {1}").format('Epu', tariffa))
        return result
...
################################################################################
    ...
    def computo_rows_list (self, supercategoria=None,
                                 categoria=None, 
                                 subcategoria=None,
                                 list_if_quantity_not_0=False,
                                 column=None,
                                 search_key=None,
...
################################################################################
    ...
    def export_epu_to_csv (self, filename, stampa_descr_estesa=False, 
        ...
        computo_csv = csv.writer(open(filename, 'w'), dialect=csv.excel, 
    ...
    def export_computo_to_csv (self, filename, stampa_descr_estesa=False, 
        ...
        # are il file csv in scrittura
        computo_csv = csv.writer(open(filename, 'w'), 
                                 dialect=csv.excel, 
                                 quoting=csv.QUOTE_NONNUMERIC)
        # scrive sul file i dati estratti e processati
    ...
    def export_to_primus_xpwe (self, filename,
                               esporta_elementi_listino=False,
                               elenco_prezzi=None,
                               remove_unused_tariffs = False):
        """esportazione del preventivo nel file di interscambio di Primus PWE
        Funzione tratta dal codice del progetto:
        LeenO - Computo Metrico
        Template assistito per la compilazione di Computi Metrici Estimativi
        Copyright (C) Giuseppe Vizziello - supporto@leeno.org"""
        
        # funzione per eliminare i caratteri speciali da eliminare nell'XML
        def parse_txt(txt):
            txt = txt.replace("&", "&amp;")
            txt = txt.replace("<", "&lt;")
            txt = txt.replace(">", "&gt;")
            ...
            txt = txt.replace('"', "&quot;")
            return txt
        # function to find the highest ID number
        def FindHighestID (parent, children):
            idls = [0]
            for itemid in parent.findall(children):
                    idls.append(int(itemid.get('ID')))
            num = max(idls)
            return num
        # costanti e inizializzazione variabili
        cr = "\r\n" # carriage return da inserire alla fine di ogni rigo
        ...
        if elenco_prezzi is None:
            ...
            tree.parse(elenco_prezzi)
            # ottieni l'item root
            top = tree.getroot() # PweDocumento
            PweDatiGenerali = top.find('PweDatiGenerali')
        # setup misurazioni
        PweMisurazioni = top.find('PweMisurazioni')
        if PweMisurazioni is None:
                PweMisurazioni = SubElement(top,'PweMisurazioni')
        # setup the document as a project (0=Listino; 1=Progetto; 2=Contabilità)
        TipoDocumento = top.find('TipoDocumento')
        if TipoDocumento is None:
            TipoDocumento = SubElement(top,'TipoDocumento')
        TipoDocumento.text = str(1)
        ##############################
        #~ dati generali
        ##############################
        if elenco_prezzi is None:
            ...
            # setup comune oggetto, etc..
            Comune.text = parse_txt(comune)
            Provincia.text = parse_txt(provincia)
            Oggetto.text = parse_txt(indirizzo)
            Committente.text = parse_txt(cliente)
            Impresa.text = parse_txt(redattore)
        ##############################
        #~ Capitoli e Categorie
        ##############################
        if elenco_prezzi is None:
            ...
            PweDGCapitoliCategorie = PweDatiGenerali.find('PweDGCapitoliCategorie')
            PweDGSuperCapitoli = PweDGCapitoliCategorie.find('PweDGSuperCapitoli')
            PweDGCapitoli = PweDGCapitoliCategorie.find('PweDGCapitoli')
            PweDGSubCapitoli = PweDGCapitoliCategorie.find('PweDGSubCapitoli')
            PweDGSuperCategorie = PweDGCapitoliCategorie.find('PweDGSuperCategorie')
            if PweDGSuperCategorie is None:
                PweDGSuperCategorie = SubElement(PweDGCapitoliCategorie,
                                                 'PweDGSuperCategorie')
            PweDGCategorie = PweDGCapitoliCategorie.find('PweDGCategorie')
            if PweDGCategorie is None:
                PweDGCategorie = SubElement(PweDGCapitoliCategorie,
                                            'PweDGCategorie')
            PweDGSubCategorie = PweDGCapitoliCategorie.find('PweDGSubCategorie')
            if PweDGSubCategorie is None:
                PweDGSubCategorie = SubElement(PweDGCapitoliCategorie,
                                               'PweDGSubCategorie')
        #~ SuperCapitoli
        sup_cap_ID = FindHighestID(PweDGSuperCapitoli,'DGSuperCapitoliItem')
        for x in sup_cap:
            (idID, desc, d, p) = x
            # if we are importing a price list update the IDs
            if elenco_prezzi is not None:
                idID += sup_cap_ID
            if idID == 0: continue
            DGSuperCapitoliItem = SubElement(PweDGSuperCapitoli,'DGSuperCapitoliItem')
            ...
            # setup the subelements tags and fields
            DGSuperCapitoliItem.set('ID', str(idID))
            DesSintetica.text = parse_txt(desc)
            DataInit.text = str(datetime.datetime.now())
            Durata.text = '0'
            Percentuale.text = '0'
        #~ Capitoli
        cap_ID = FindHighestID(PweDGCapitoli,'DGCapitoliItem')
        for x in cap:
            # if we are importing a price list do not create chapters
            (idID, desc, d, p) = x
            # if we are importing a price list update the IDs
            if elenco_prezzi is not None:
                idID += cap_ID
            if idID == 0: continue
            DGCapitoliItem = SubElement(PweDGCapitoli,'DGCapitoliItem')
            ...
            # setup the subelements tags and fields
            DGCapitoliItem.set('ID', str(idID))
            DesSintetica.text = parse_txt(desc)
            DataInit.text = str(datetime.datetime.now())
            Durata.text = '0'
            Percentuale.text = '0'
        #~ SubCapitoli
        sub_cap_ID = FindHighestID(PweDGSubCapitoli,'DGSubCapitoliItem')
        for x in sub_cap:
            (idID, desc, d, p) = x
            # if we are importing a price list update the IDs
            if elenco_prezzi is not None:
                idID += sub_cap_ID
            if idID == 0: continue
            DGSubCapitoliItem = SubElement(PweDGSubCapitoli,'DGSubCapitoliItem')
            ...
            DGSubCapitoliItem.set('ID', str(idID))
            DesSintetica.text = parse_txt(desc)
            DataInit.text = str(datetime.datetime.now())
            Durata.text = '0'
            Percentuale.text = '0'
        #~ SuperCategorie
        sup_cat_ID = FindHighestID(PweDGSuperCategorie,'DGSuperCategorieItem')
        for x in sup_cat:
            (idID, desc, d, p) = x
            # if we are importing a price list update the IDs
            if elenco_prezzi is not None:
            ...
            # setup the subelements tags and fields
            DGSuperCategorieItem.set('ID', str(idID))
            DesSintetica.text = parse_txt(desc)
            DataInit.text = str(datetime.datetime.now())
            Durata.text = '0'
            Percentuale.text = '0'
        #~ Categorie
        cat_ID = FindHighestID(PweDGCategorie,'DGCategorieItem')
        for x in cat:
            (idID, desc, d, p) = x
            # if we are importing a price list update the IDs
            if elenco_prezzi is not None:
            ...
            # setup the subelements tags and fields
            DGCategorieItem.set('ID', str(idID))
            DesSintetica.text = parse_txt(desc)
            DataInit.text = str(datetime.datetime.now())
            Durata.text = '0'
            Percentuale.text = '0'
        #~ SubCategorie
        sub_cat_ID = FindHighestID(PweDGSubCategorie,'DGSubCategorieItem')
        for x in sub_cat:
            (idID, desc, d, p) = x
            # if we are importing a price list update the IDs
            if elenco_prezzi is not None:
            ...
            DGSubCategorieItem.set('ID', str(idID))
            DesSintetica.text = parse_txt(desc)
            DataInit.text = str(datetime.datetime.now())
            Durata.text = '0'
            Percentuale.text = '0'
        ...
        if elenco_prezzi is None:
            ...
            # Spese e Utili: se Accorpati '1', altrimenti '-1'
            SpeseUtili.text = '-1'
            UtiliImpresa.text = str(utile_impresa)
            OneriAccessoriSc.text = str(sicurezza)
            SpeseGenerali.text = str(spese_generali)
        ##############################
        #~ Configurazioni
        ##############################
        if elenco_prezzi is None:
        ...
        #~ Elenco Prezzi
        ##############################
        PweElencoPrezzi = PweMisurazioni.find('PweElencoPrezzi')
        if PweElencoPrezzi is None:
            PweElencoPrezzi = SubElement(PweMisurazioni,'PweElencoPrezzi')
        nEPItem = FindHighestID(PweElencoPrezzi, 'EPItem')+1
        EP_key = dict()
        # create a list of existing items
        l_tar = list()
        for tariffa in lista_tariffe:
            l_tar.append(tariffa[0])
        for EPItem in PweElencoPrezzi.findall('EPItem'):
            tariffa = EPItem.find('Tariffa')
            if tariffa is None:
                raise PreventiviError("""Voce ID {} priva di Tariffa""".format(
                                      int(EPItem.get('ID'))))
            elif tariffa.text in l_tar:
                EP_key[tariffa.text] = int(EPItem.get('ID'))
            elif tariffa.text not in l_tar and remove_unused_tariffs:
                # remove the item not used
                logging.debug("Removing unused tariff: '{}'".format(tariffa.text))
                PweElencoPrezzi.remove(EPItem)
        # parse the list of EPU and add to the document
        for articolo in lista_articoli_epu:
            # check if the tariff already exists
            if articolo.tariffa in EP_key.keys():
                logging.debug ("Tariff '{}' already exists".format(articolo.tariffa))
                # we don't need to add this item, go to the next
                continue
            else:
                EPItem = SubElement(PweElencoPrezzi,'EPItem')
                EPItem.set('ID', str(nEPItem))
                EP_key[articolo.tariffa] = nEPItem
                nEPItem += 1
                # complete the construction of the element
                TipoEP = SubElement(EPItem,'TipoEP')
                TipoEP.text = '0' # 0:Voce semplice; 1:V.di Gruppo; 2:V.Sottogruppo
                Tariffa = SubElement(EPItem,'Tariffa')
                Tariffa.text = articolo.tariffa
                Articolo = SubElement(EPItem,'Articolo')
                Articolo.text = articolo.tariffa
                DesEstesa = SubElement(EPItem,'DesEstesa')
                DesEstesa.text = parse_txt(articolo.descrizione_estesa)
                DesRidotta = SubElement(EPItem,'DesRidotta')
                DesRidotta.text = parse_txt(articolo.descrizione_codice)
                DesBreve = SubElement(EPItem,'DesBreve')
                DesBreve.text = parse_txt(articolo.descrizione_codice)
                UnMisura = SubElement(EPItem,'UnMisura')
                UnMisura.text = articolo.unita_misura
                Prezzo1 = SubElement(EPItem,'Prezzo1')
                Prezzo1.text = str(articolo.prezzo_unitario)
                Prezzo2 = SubElement(EPItem,'Prezzo2')
                Prezzo2.text = str(articolo.costo_mat1)
                Prezzo3 = SubElement(EPItem,'Prezzo3')
                Prezzo3.text = str(articolo.costo_mat2)
                Prezzo4 = SubElement(EPItem,'Prezzo4')
                Prezzo4.text = str(articolo.costo_mat3)
                Prezzo5 = SubElement(EPItem,'Prezzo5')
                Prezzo5.text = str(articolo.costo_mat4)
                IDSpCap = SubElement(EPItem,'IDSpCap')
                IDSpCap.text = str(articolo.supercapitolo + sup_cap_ID)
                IDCap = SubElement(EPItem,'IDCap')
                IDCap.text = str(articolo.capitolo + cap_ID)
                IDSbCap = SubElement(EPItem,'IDSbCap')
                IDSbCap.text = str(articolo.subcapitolo + sub_cap_ID)
                Flags = SubElement(EPItem,'Flags')
                # Flags DWORD (32 bit)
    	        # bit 10: SOLO DESCRIZIONE;
	            # bit 12: VOCE DEL FERRO;
                # bit 17: ANALISI;
                # Settando uno o più bit e possibile settare le proprietà alla voce
                Flags.text = '0'
                # sistemazione della data x primus
                if articolo.data is None:
                    articolo.data = str(datetime.datetime.now())
                Data = SubElement(EPItem,'Data')
                Data.text = "{0}/{1}/{2}".format(articolo.data[8:10], 
                                             articolo.data[5:7], 
                                             articolo.data[0:4])
                AdrInternet = SubElement(EPItem,'AdrInternet')
                AdrInternet.text = ''
                IncSIC = SubElement(EPItem,'IncSIC')
                IncSIC.text = str(articolo.sicurezza)
                #TODO incidenza materiali e manodopera
                ...
                # Analisi di prezzo
                ##############################
                PweEPAnalisi = SubElement(PweElencoPrezzi,'PweEPAnalisi')
                nEPARItem = 2
                for lis in lista_listino:
                    if lis.tariffa != articolo.tariffa:
                        continue
                    PweEPAR = SubElement(PweEPAnalisi,'PweEPAR')
                    PweEPAR.set('ID', str(nEPARItem))
                    nEPARItem += 1
                    Tipo = SubElement(PweEPAR,'Tipo')
                    Tipo.text = '0'
                    IDEP = SubElement(PweEPAR,'IDEP')
                    IDEP.text = '-2'
                    Descrizione = SubElement(PweEPAR,'Descrizione')
                    Descrizione.text = parse_txt(str(lis.descrizione_codice))
                    Misura = SubElement(PweEPAR,'Misura')
                    Misura.text = str(lis.unita_misura)
                    Qt = SubElement(PweEPAR,'Qt')
                    Qt.text = str(lis.quantita)
                    Prezzo = SubElement(PweEPAR,'Prezzo')
                    Prezzo.text = str(lis.prezzo_unitario)
                    FieldCTL = SubElement(PweEPAR,'FieldCTL')
                    FieldCTL.text = '0'
        ##############################
        # COMPUTO/VARIANTE
        ##############################
        # create the subelement for bill of quantities
        PweVociComputo = PweMisurazioni.find('PweVociComputo')
        if PweVociComputo is None:
            PweVociComputo = SubElement(PweMisurazioni,'PweVociComputo')
        nVCItem = FindHighestID(PweVociComputo, 'VCItem')+1
        # create a selection of elements to add in PweVociComputo 
        self.c.execute("""SELECT Tariffa, Supercategoria, """
                       """Categoria, Subcategoria FROM Computo """
                       """ORDER BY id;""")
        lis_cmp = set(self.c.fetchall())
        # add each line in database to voci computo
        for prymary_key in lista_id_cmp:
            articolo = self.copia_articolo_computo (prymary_key[0])
            art_sign = (articolo.tariffa,
                        articolo.supercategoria,
                        articolo.categoria,
                        articolo.subcategoria)
            # if the article is in the list, remove from list and proceed adding
            # the article in the XML structure
            if art_sign in lis_cmp:
                    lis_cmp.remove(art_sign)
            else:
                # if the article is not in the list it was already added
                logging.debug("""Article with primary key '{}' already """
                              """added, I skip it.""".format(prymary_key))
                continue
            ...
            # get the id from EPU
            IDEP.text = str(EP_key[articolo.tariffa])
            Quantita = SubElement(VCItem,'Quantita')
            Quantita.text = ''
            DataMis = SubElement(VCItem,'DataMis')
            DataMis.text = str(datetime.datetime.now())
            vFlags = SubElement(VCItem,'Flags')
            vFlags.text = '0'
            IDSpCat = SubElement(VCItem,'IDSpCat')
            IDSpCat.text = str(articolo.supercategoria + sup_cat_ID)
            IDCat = SubElement(VCItem,'IDCat')
            IDCat.text = str(articolo.categoria + cat_ID)
            IDSbCat = SubElement(VCItem,'IDSbCat')
            IDSbCat.text = str(articolo.subcategoria + sub_cat_ID)
            ##########################
            # righe di misurazione
            ##########################
            # create a list of mesures to add
            # FIXME errore nelle quantità di ogni articolo!!!
            key_dict = {"tar":    articolo.tariffa,
                        "SupCat": articolo.supercategoria,
                        "Cat":    articolo.categoria,
                        "SubCat": articolo.subcategoria}
            self.c.execute("""SELECT Tariffa,Quantita,Note FROM Computo """
                           """WHERE Tariffa=:tar AND Supercategoria=:SupCat """
                           """AND Categoria=:Cat AND Subcategoria=:SubCat """
                           """ORDER BY id;""", key_dict)
            lis_min = self.c.fetchall()
            if len(lis_min) == 0:
                raise PreventiviError("List of Computo Measure is empty")
            # create the measure subelement
            PweVCMisure = SubElement(VCItem,'PweVCMisure')
            nVMItem = 1
            # add each line of measure and its quantity
            for m,quant,note in lis_min:
                RGItem = SubElement(PweVCMisure,'RGItem')
                RGItem.set('ID', str(nVMItem))
                nVMItem += 1
                IDVV = SubElement(RGItem,'IDVV')
                IDVV.text = '-2'
                Descrizione = SubElement(RGItem,'Descrizione')
                Descrizione.text = parse_txt(str(note))
                PartiUguali = SubElement(RGItem,'PartiUguali')
                PartiUguali.text = str(quant)
    ...
    def export_to_SIX (self, filename, elenco_prezzi=None):
        """esportazione del preventivo nel file di interscambio STR SIX"""
        # function to find the highest ID number
        def FindHighestID (parent, children, attribute):
            idls = [0]
            if parent.findall(children) is None:
                return 0
            for itemid in parent.findall(children):
                    idls.append(int(itemid.get(attribute)))
            num = max(idls)
            return num
        # ricava una lista di oggetti 'articolo' ricavata dalla tabella epu
        lista_articoli_epu = list()
        ...
        if elenco_prezzi is None:
            # crea la struttura XML
            top = Element('Documento')
            tree._setroot(top)
        else:
            # effettua il parsing del file XML
            tree.parse(elenco_prezzi)
            # ottieni l'item root
            top = tree.getroot() # Documento
        ##############################
        # intestazione
        ##############################
        if elenco_prezzi is None:
            intestazione = SubElement(top,'intestazione')
            # dati intestazione
            intestazione = SubElement(intestazione,'PweDGProgetto')
            intestazione.set('intestazioneId', "1")
            intestazione.set('lingua', "it")
            intestazione.set('separatore', ".")
            intestazione.set('separatoreParametri', ";")
            intestazione.set('valuta', "EUR")
            intestazione.set('autore', redattore)
            intestazione.set('versione', "")
        else:
            intestazione = top.find('intestazione')
            if redattore != "" and redattore is not None:
                intestazione.set('autore', redattore)
        # setup common variables
        TipoValuta = "EUR" # USD, GBP
        TipoCategoriaPrezzario = "EPU" # Listino
        ##############################
        # for now ignore: quadroEconomico, motivoDiSospensione
        ##############################
        pass
        ##############################
        # categoriaSOA
        ...
        soa_dict = dict()
        if elenco_prezzi is None:
            categoriaSOA = SubElement(top,'categoriaSOA')
            # compire if required
            pass
        else:
            categoriaSOA = top.find('categoriaSOA')
            if categoriaSOA is not None:
                for cat in categoriaSOA:
                    soaId = cat.get('soaId')
                    soaCategoria = cat.get('soaCategoria')
                    soa_dict[soaId] = soaCategoria
        ##############################
        # for now ignore: diametro, gruppo, formula, sistema
        ##############################
        if elenco_prezzi is None:
            pass
        else:
            diametro = top.find('diametro')
            formula = top.find('formula')
            sistema = top.find('sistema')
            gruppo = top.find('gruppo')
        ##############################
        # prezzario
        ##############################
        prezzario = top.find('prezzario')
        if prezzario is None:
            prezzario = SubElement(top,'prezzario')
            prezzario.set('prezzarioId', "1")
            prezzario.set('przId', "1")
            prezzario.set('prdStruttura', "zzzzzz.zzz.zzzzzz.zzz.zzz")
            prezzario.set('valuta', TipoValuta)
            prezzario.set('categoriaPrezzario', TipoCategoriaPrezzario)
            prezzario.set('arrotondamento', "0.01")
            prezzario.set('arrotondamento', "0.01")
            prezzario.set('arrotondamentoImporto', "0.01")
            prezzario.set('arrotondamentoPercentuale', "0.01")
            # crea lista quotazione base
            listaQuotazione = SubElement(prezzario,'listaQuotazioneId')
            listaQuotazione.set('listaQuotazioneId', "1")
            listaQuotazione.set('lqtId', "1")
            listaQuotazione.set('valuta', TipoValuta)
            # create at least one unitaDiMisura
            unitaDiMisura = SubElement(prezzario,'unitaDiMisura')
            unitaDiMisura.set('unitaDiMisuraId', "1")
            unitaDiMisura.set('udmId', "1")
            unitaDiMisura.set('simbolo', "cad.")
            unitaDiMisura.set('decimali', "2")
        # trova la prima lista quotazione
        listaQuotazioneId = prezzario.find('listaQuotazione')[0].get('listaQuotazioneId')
        # create a dictionary of unitaDiMisura
        udm_dict = dict()
        for udm in prezzario.find('unitaDiMisura'):
            u_id = udm.get("unitaDiMisuraId")
            sym = udm.get("simbolo")
            udm_dict[u_id] = sym
        # prodotto
        nEPItem = FindHighestID(prezzario, prodotto, 'prodottoId')+1
        EP_key = dict()
        # create a list of existing items
        l_tar = list()
        for tariffa in lista_tariffe:
            l_tar.append(tariffa[0])
        for prodotto in prezzario.findall('prodotto'):
            prodottoId = prodotto.get('prodottoId') # nr. in sequenza
            prdId = prodotto.get('prdId') # tariffa
            if prdId is None:
                raise PreventiviError("""Voce ID {} priva di Tariffa""".format(
                                      prodottoId))
            elif prdId in l_tar:
                EP_key[prdId] = int(prodottoId)
        # parse the list of EPU and add to the document
        for articolo in lista_articoli_epu:
            # check if the tariff already exists
            if articolo.tariffa in EP_key.keys():
                ...
                prodotto = SubElement(prezzario, 'prodotto')
                prodottoId = prodotto.set('prodottoId', nEPItem)
                prdId = prodotto.set('prdId', articolo.tariffa)
                nEPItem += 1
                udm = udm_dict[articolo.unita_misura]
                if udm_dict.has_key(articolo.unita_misura):
                    prodotto.set('unitaDiMisuraId',
                                 udm_dict[articolo.unita_misura])
                else:
                    # get last unitaDiMisura index
                    udmindex = FindHighestID(prezzario, unitaDiMisura, 
                                             'unitaDiMisuraId')+1
                    # append unitaDiMisura
                    unitaDiMisura = SubElement(prezzario,'unitaDiMisura')
                    unitaDiMisura.set('unitaDiMisuraId', udmindex)
                    unitaDiMisura.set('udmId', udmindex)
                    unitaDiMisura.set('simbolo', articolo.unita_misura)
                    unitaDiMisura.set('decimali', "2")
                # descrizione voce
                prdDescrizione = SubElement(prodotto,'prdDescrizione')
                prdDescrizione.set('lingua', "it")
                prdDescrizione.set('breve', articolo.descrizione_codice)
                prdDescrizione.set('estesa', articolo.descrizione_estesa)
                # prdQuotazione
                prdQuotazione = SubElement(prodotto,'prdQuotazione')
                prdQuotazione.set('valore', articolo.prezzo_unitario)
                #prdQuotazione.set('validita', datetime.datetime.now)
                prdQuotazione.set('listaQuotazioneId', listaQuotazioneId)
        ##############################
        # progetto
        ##############################
        progetto = top.find('progetto')
        if progetto is None:
            progetto = SubElement(top,'progetto')
            progetto.set('prgId', "1")
            prgPrezzario = SubElement(progetto,'prgPrezzario')
            prgPrezzario.set("prezzarioId", prezzario.get("prezzarioId"))
        #FIXME nVCItem = FindHighestID(PweVociComputo, 'VCItem')+1
        # create a selection of elements to add in 'progetto' 
        self.c.execute("""SELECT Tariffa, Supercategoria, """
                       """Categoria, Subcategoria FROM Computo """
                       """ORDER BY id;""")
        lis_cmp = set(self.c.fetchall())
...
################################################################################
    ...
    def import_from_primus_xpwe (self, filename):
        """Routine di importazione da file formato XML di Primus XPWE
        Funzione tratta dal codice del progetto:
        LeenO - Computo Metrico
        Template assistito per la compilazione di Computi Metrici Estimativi
        Copyright (C) Giuseppe Vizziello - supporto@leeno.org"""
        def ParseFloat(value):
            ...
            try:
                ...
                raise PreventiviError("Error converting to float {}".format(value))
        lista_articoli = list() # lista in cui memorizzare gli articoli da importare
        lista_analisi = list() # lista articoli di analisi prezzi
        diz_ep = dict() # array per le voci di elenco prezzi
        # lista capitoli e categorie (id, descrizione, percentuale)
        lista_supcap = list()
        lista_cap = list()
        lista_subcap = list()
        lista_supcat = list()
        ...
        # cliente=None,ricarico=None,manodopera=None,sicurezza=None,indirizzo=None, 
        self.update_dati_generali (nome=oggetto, 
                                   redattore=impresa, 
                                   comune=comune, 
                                   provincia=provincia)
        # ottengo la 'root' delle misurazioni e voci di EPU
        PweMisurazioni = root.find('PweMisurazioni')
        ########################################################################
        # leggo l'elenco prezzi ################################################
        ########################################################################
        PweElencoPrezzi = PweMisurazioni.getchildren()[0]
        ...
        for elem in epitems:
            ...
            if tariffa is not None:
                pass
            elif tariffa is None and articolo is not None:
                tariffa = articolo
            else:
                tariffa = id_ep
            ...
            art = ArticoloComputo(self.db, 
                          ...
                          0, # subcategoria 
                          tariffa,
                          descrizione_codice = desridotta, 
                          descrizione_voce = desbreve,
                          descrizione_estesa = destestesa, 
                          unita_misura = unmisura, 
                          prezzo_unitario = ParseFloat(prezzo1),
                          costo_mat1 = ParseFloat(prezzo2),
                          costo_mat2 = ParseFloat(prezzo3),
                          costo_mat3 = ParseFloat(prezzo4),
                          costo_mat4 = ParseFloat(prezzo5),
                          quantita=0)
            # aggiungi l'articolo nel dizionario
            diz_ep[id_ep] = art
            ####################################################################
            ...
            if PweEPAR is not None:
                ...
                for el in EPARItem:
                    ...
                    # crea la voce di listino
                    lis = ArticoloListino(tariffa = tariffa, 
                                      codice = an_tipo, 
                                      descrizione_codice = an_des,
                                      unita_misura = an_um, 
                                      quantita = ParseFloat(an_qt), 
                                      prezzo_unitario = ParseFloat(an_pr), 
                                      accessori = None, 
                                      sconto = 0, 
                                      note = an_fld)
                    # aggiungi la voce di listino alla lista
                    lista_analisi.append(lis)
        ########################################################################
        # leggo voci di COMPUTO ################################################
        ########################################################################
        PweVociComputo = PweMisurazioni.getchildren()[1]
        ...
        for elem in vcitems:
            ...
            for el in righi_mis:
                ...
                #TODO aggiungere le note di misurazione??
            # recupero la voce di EPU precedentemente creata e aggiungo la voce
            artcm = diz_ep[id_ep]
            # modifico la voce aggiungendo categorie e nomi
            artcm.supercategoria = idspcat
            artcm.categoria = idcat
            artcm.subcategoria = idsbcat
            # modifico la quantità
            artcm.quantita = ParseFloat(quantita)
            # aggiungo la voce alla lista
            lista_articoli.append(artcm)
        # importazione degli articoli nel database
        logger.debug("Importazione di {0} articoli nel database".format(len(lista_articoli)))
        self.insert_articoli_computo(*lista_articoli)
        # aggiungi gli articoli di listino
        self.insert_articoli_listino(None, *lista_analisi)
        ########################################################################
        ...
        #PweDGSuperCapitoli #
        #####################
        if True: #if 'PweDGSuperCapitoli' in CapCat:
            ...
            for elem in PweDGSuperCapitoli:
                ...
                lista_supcap.append((id_sc, dessintetica))
        ################
        #PweDGCapitoli #
        ################
        if True: #if 'PweDGCapitoli' in CapCat:
            ...
            for elem in PweDGCapitoli:
                ...
                dessintetica = elem.find('DesSintetica').text
                #percentuale = elem.find('Percentuale').text
                lista_cap.append((id_sc, dessintetica))
        ###################
        #PweDGSubCapitoli #
        ###################
        if True: #if 'PweDGSubCapitoli' in CapCat:
            ...
            for elem in PweDGSubCapitoli:
                ...
                dessintetica = elem.find('DesSintetica').text
                #percentuale = elem.find('Percentuale').text
                lista_subcap.append((id_sc, dessintetica))
        ######################
        #PweDGSuperCategorie #
        ######################
        if True:#if 'PweDGSuperCategorie' in CapCat:
            PweDGSuperCategorie = CapCat.find('PweDGSuperCategorie').getchildren()
            for elem in PweDGSuperCategorie:
                id_sc = elem.get('ID')
                dessintetica = elem.find('DesSintetica').text
                #percentuale = elem.find('Percentuale').text
                lista_supcat.append((id_sc, dessintetica))
        #################
        #PweDGCategorie #
        #################
        if True: #if 'PweDGCategorie' in CapCat:
            PweDGCategorie = CapCat.find('PweDGCategorie').getchildren()
            for elem in PweDGCategorie:
                id_sc = elem.get('ID')
                dessintetica = elem.find('DesSintetica').text
                #percentuale = elem.find('Percentuale').text
                lista_cat.append((id_sc, dessintetica))
        ####################
        #PweDGSubCategorie
        ####################
        if True: #if 'PweDGSubCategorie' in CapCat:
            PweDGSubCategorie = CapCat.find('PweDGSubCategorie').getchildren()
            for elem in PweDGSubCategorie:
                id_sc = elem.get('ID')
                dessintetica = elem.find('DesSintetica').text
                #percentuale = elem.find('Percentuale').text
                lista_subcat.append((id_sc, dessintetica))
        
        # effettua un update dei nomi dei capitoli
        def aggiorna_cap_cat(lista, Tipo):
            logger.debug(Tipo)
            logger.debug(lista)
            for key, Nome in lista:
                if int(key) == 0:
                    continue
                self.update_capitoli_categorie (key, Tipo, Nome)
        aggiorna_cap_cat(lista_supcap, "Supercapitolo")
        aggiorna_cap_cat(lista_cap,    "Capitolo")
        aggiorna_cap_cat(lista_subcap, "Subcapitolo")
        aggiorna_cap_cat(lista_supcat, "Supercategoria")
        aggiorna_cap_cat(lista_cat,    "Categoria")
        aggiorna_cap_cat(lista_subcat, "Subcategoria")
        #FIXME verificare errori nei nomi capitoli
...
################################################################################
    ...
    def import_XML (self, filename):
        ...
        desc_estesa = str()
        # effettua il parsing del file XML
        logger.debug(_("Parsing del file XML: {0}").format(filename))
        tree = ElementTree()
        tree.parse(filename)
        # ottieni l'item root
        root = tree.getroot()
        logger.debug(list(root))
            ...
            # se il tag è un prodotto fa parte degli articoli da analizzare
                ...
                if elem.get("titolo") == "true" and sub_desc is not None and sub_quot is None:
                    ...
                    if livello == 1:
                        ...
                        logger.debug(_("SUPERCAPITOLO: {0}").format(titolo_sup))
                    elif livello == 2:
                        titolo_cap = titolo
                        if len(titolo_cap) > 70: titolo_cap = "{0}...".format(titolo_cap[:67])
                        logger.debug(_("CAPITOLO: {0}").format(titolo_cap))
                    elif livello == 3:
                        titolo_sub = titolo
                        if len(titolo_sub) > 70: titolo_sub = "{0}...".format(titolo_sub[:67])
                        logger.debug(_("SUBCAPITOLO: {0}").format(titolo_sub))
...
class ArticoloComputo(Preventivo):
    ...
    def set_unit_price(self, value):
        ...
        if it would be negative lower the materials cost.
        """
        ...
        # definisco le procedure di calcolo del tempo e della manodopara
        def calcolo_tempo(value):
            ...
            try:
                ...
                self.tempo_inst = ((value / (1+(self.sicurezza/100))) - (self.costo_materiali * self.ricarico)) * (60/costo_mo_ora)
            except ZeroDivisionError:
                self.tempo_inst = 0
            finally:
                if self.tempo_inst < 0: self.tempo_inst = 0
                # ricalcolo il nuovo prezzo unitario
                self.prezzo_unitario = self.__calcola_prezzo_unitario ()
        # procedure di calcolo dei costi materiali
        def calcolo_mat(value):
            try:
                ...
                # ricalcolo il nuovo prezzo unitario
                self.prezzo_unitario = self.__calcola_prezzo_unitario ()
        # se il costo orario della manodopera è <= 0 varia il costo dei materiali
        # se il costo orario della manodopera è > 0 cambia il valore del tempo
        # di installazione a meno che questo non sia già negativo e ripete il ciclo
        while self.prezzo_unitario != value:
            if costo_mo_ora <= 0:
                calcolo_mat(value)
            elif costo_mo_ora > 0 and self.tempo_inst > 0:
                calcolo_tempo(value)
            elif costo_mo_ora > 0 and self.tempo_inst <= 0:
                self.tempo_inst = 0
                calcolo_mat(value)
            else: break
            #FIXME esiste ancora la possibilità di loop?
    ...
    def __str__(self):
        ...
        if self.art_listino != []:
            for articolo_listino in self.art_listino:
                art_listino.append(str(articolo_listino))
        ...
        stringa = """ArtCmp|%s|%s|%s|%s|%s|%d|%s|%d|%s|%d|%s|%d|%s|%d|%s|%d|%s|%s|%f|%f|%f|%f|%f|%f|%s|%s|%s|%f|%f|%f|%f|%f|%s|%s|%s""" % (
                  self.tariffa, codice, self.descrizione_codice, 
                  ...
                  self.cat_appalto, repr(art_listino))
        return stringa
...
class ArticoloListino(Preventivo):
    ...
    def __str__(self):
        ...
        stringa = "ArtLis§%s§%s§%s§%s§%f§%f§%f§%f§%s§%s§%f" % (self.tariffa, 
            self.codice, self.descrizione_codice,
            self.unita_misura, self.quantita, 
            self.prezzo_unitario, self.accessori, 
